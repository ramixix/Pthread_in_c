...
int fuel = 50;
/*
`   Note : when using recursive mutex think of them as normal mutex, it doesn't important how many times
    one thread locks the mutex, if one thread lock the mutex before the others, the other threads wont be able
    to lock the mutex until that thread unlocks it, So if thread locks the mutex 3 times then that thread
    need to unlock that mutex for 3 times before any other thread can actually lock the same mutex again. (the same as when we use just 1 mutex) 
...
int main(){
    ...
    /*
        ...
        Then initial that created variable with pthread_mutexattr_init(&mutex_attr_var);
        ...
        and pass the attribute variable to the pthread_mutex_init(&mutex, &attributes) function.
